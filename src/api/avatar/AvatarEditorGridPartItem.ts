...
export class AvatarEditorGridPartItem implements IAvatarImageListener {
    ...
    constructor(
        ...
        if (partSet) {
            ...
            for (const color of colors)
                this._maxColorIndex = Math.max(
                    this._maxColorIndex,
                    color.colorLayerIndex
                );
        }
    }
    ...
    private analyzeFigure(): boolean {
        if (
            ...
            !this._partSet ||
            !this._partSet.parts ||
            !this._partSet.parts.length
        )
            return false;

        const figureContainer = this._renderManager.createFigureContainer(
            this.partSet.type + '-' + this.partSet.id
        );

        if (!this._renderManager.isFigureContainerReady(figureContainer)) {
            // 🔥 downloads async; MUST rely on avatarImageReady to re-render later
    ...
    private renderThumb(): NitroContainer {
        ...
        for (const part of parts) {
            ...
            while (
                !hasAsset &&
                direction < AvatarEditorGridPartItem.THUMB_DIRECTIONS.length
            ) {
                const assetName =
                    FigureData.SCALE +
                    '_' +
    ...
    // ✅ THIS IS WHAT MAKES THUMBS APPEAR AFTER DOWNLOAD
    public avatarImageReady(figure: string): void {
        if (this._disposed) return;

        // assets ready -> re-check + rebuild
        this._isValidFigure = false;
        this.resetFigure(figure);
