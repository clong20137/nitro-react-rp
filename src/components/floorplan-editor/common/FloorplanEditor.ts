import {
    ...
    NitroPoint,
    NitroTilemap,
    PixiApplicationProxy,
    PixiInteractionEventProxy,
    POINT_STRUCT_SIZE,
...
export class FloorplanEditor extends PixiApplicationProxy {
    ...
    private registerEventListeners(): void {
        ...
        this._tilemapRenderer.containsPoint = (position) => {
            this._tilemapRenderer.worldTransform.applyInverse(
                position,
                tempPoint
            );
        ...
        this._tilemapRenderer.on(
            'pointerdown',
            (event: PixiInteractionEventProxy) => {
                if (
                    !(event.data.originalEvent instanceof PointerEvent) &&
                    !(event.data.originalEvent instanceof TouchEvent)
                )
                    return;
    ...
    private tileHitDetection(
        tempPoint: NitroPoint,
        setHolding: boolean,
        ...
        for (let j = 0; j < len; j += bufSize) {
            ...
            if (solution) {
                if (this._isHolding) {
                    const [realX, realY] = getTileFromScreenPosition(
                        tileStartX,
                        tileStartY
                    );
    ...
    private onClick(x: number, y: number): void {
        ...
        switch (this._actionSettings.currentAction) {
            ...
            case FloorAction.SET:
                futureHeightIndex = HEIGHT_SCHEME.indexOf(
                    this._actionSettings.currentHeight
                );
                break;
        ...
        for (let y = 0; y < this._tilemap.length; y++) {
            for (let x = 0; x < this.tilemap[y].length; x++) {
                ...
                this._tilemapRenderer.tile(
                    this._assetCollection.getTexture(
                        `floor_editor_${assetName}`
                    ),
                    positionX,
                    positionY
                );
    ...
    public setTilemap(map: string, blockedTiles: boolean[][]): void {
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                const blocked =
                    (blockedTiles[y] && blockedTiles[y][x]) || false;
