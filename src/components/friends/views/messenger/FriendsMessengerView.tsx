...
export const FriendsMessengerView: FC<{}> = (props) => {
    ...
    const followFriend = () =>
        ...
        SendMessageComposer(
            new FollowFriendMessageComposer(activeThread.participant.id)
        );
    const openProfile = () =>
        activeThread &&
        activeThread.participant &&
    ...
    useEffect(() => {
        ...
        // When messenger window is hidden, remember the last thread
        // but DO NOT clear it, so PhoneView can still use it.
        if (!isVisible && activeThread) {
        ...
        >
            <NitroCardHeaderView
                headerText={LocalizeText(
                    ...
                    ["OPEN_CHAT_COUNT"],
                    [visibleThreads.length.toString()]
                )}
                            ...
                            {visibleThreads &&
                                ...
                                visibleThreads.map((thread) => {
                                    return (
                                        <LayoutGridItem
                                            key={thread.threadId}
                                            itemActive={activeThread === thread}
                                            onClick={(event) =>
                                                setActiveThreadId(
                                                    thread.threadId
                                                )
                            ...
                            <>
                                ...
                                >
                                    <Flex gap={1}>
                                        ...
                                        >
                                            {LocalizeText(
                                                "messenger.window.button.report"
                                            )}
                                        </Button>
                                    </Flex>
                                    <Button
