...
export class RoomSpriteCanvas implements IRoomRenderingCanvas {
    ...
    constructor(
        container: IRoomSpriteCanvasContainer,
        id: number,
        width: number,
        height: number,
        scale: number
        ...
        this._animationFPS = Nitro.instance.getConfiguration<number>(
        ...
        this._objectCache = new RoomObjectCache(
            this._container.roomObjectVariableAccurateZ
        );
    ...
    public render(time: number, update: boolean = false): void {
        ...
        if (
            ...
            this._height !== this._renderedHeight
        )
            update = true;
        ...
        const frame = Math.round(
            this._totalTimeRunning / (60 / this._animationFPS)
        );
    ...
    private renderObject(
        ...
        const visualization =
            object.visualization as IRoomObjectSpriteVisualization;
        ...
        if (
            !sortableCache.needsUpdate(
                visualization.instanceId,
                visualization.updateSpriteCounter
            ) &&
            !update
        ) {
    ...
    private renderSprite(index: number, sprite: SortableSprite): boolean {
        ...
        if (
            extendedSprite.needsUpdate(
                objectSprite.id,
                objectSprite.updateCounter
            ) ||
            ...
            (extendedSprite as any).__spriteType = objectSprite.spriteType;
            ...
            if (extendedSprite.texture !== objectSprite.texture)
    ...
    private createAndAddSprite(
        ...
        (extendedSprite as any).__spriteType = sprite.spriteType;
    ...
    public handleMouseEvent(
        ...
        if (this._mouseCheckCount > 0 && type == MouseEventType.MOUSE_MOVE)
    ...
    private checkMouseHits(
        ...
        const clickThrough = (window as any).isClickThroughUsers?.() === true;

        while (spriteId >= 0) {
            const extendedSprite = this.getExtendedSprite(spriteId);
            if (clickThrough && extendedSprite) {
                const st = (extendedSprite as any).__spriteType;
                if (
                    st === RoomObjectSpriteType.AVATAR ||
                    st === RoomObjectSpriteType.AVATAR_OWN
                ) {
                    spriteId--;
                    continue; // ignore avatars and keep scanning lower sprites
                }
            }

            if (
                extendedSprite &&
                extendedSprite.containsPoint(
                    new Point(x - extendedSprite.x, y - extendedSprite.y)
                )
            ) {
                if (
                    ...
                    const identifier =
                        this.getExtendedSpriteIdentifier(extendedSprite);

                    if (checkedSprites.indexOf(identifier) === -1) {
                        const tag = extendedSprite.tag;

                        let mouseData =
                            this._mouseActiveObjects.get(identifier);

                        if (mouseData) {
                            if (mouseData.spriteTag !== tag) {
                                mouseEvent = this.createMouseEvent(
                                    0,
                                    0,
                                    0,
                                    0,
                        ...
                        if (
                            type === MouseEventType.MOUSE_MOVE &&
                            (!mouseData || mouseData.spriteTag !== tag)
                        ) {
                            mouseEvent = this.createMouseEvent(
                                x,
                                y,
    ...
    protected processMouseEvents(): void {
        ...
        for (const [key, event] of this._eventCache.entries()) {
            ...
            if (this._mouseListener) {
                this._mouseListener.processRoomCanvasMouseEvent(
                    event,
                    roomObject,
                    this._geometry
                );
    ...
    private doMagic(): void {
        ...
        if (this._rotation !== 0) {
            ...
            geometry.direction = new Vector3d(
                direction.x + this._rotation,
                direction.y,
                direction.z
            );
            ...
            location.x =
                ...
                this._rotationRodLength *
                    Math.cos(((direction.x + 180) / 180) * 3.14159265358979) *
                    Math.cos((direction.y / 180) * 3.14159265358979);
            location.y =
                location.y +
                this._rotationRodLength *
                    Math.sin(((direction.x + 180) / 180) * 3.14159265358979) *
                    Math.cos((direction.y / 180) * 3.14159265358979);
            location.z =
                location.z +
                this._rotationRodLength *
                    Math.sin((direction.y / 180) * 3.14159265358979);
        ...
        if (this._SafeStr_4507) {
            ...
            const _local_1 = Vector3d.sum(
                _local_4,
                new Vector3d(
                    Math.sin(
                        ((this._SafeStr_795 * 5) / 180) * 3.14159265358979
                    ) * 2,
                    Math.sin((this._SafeStr_795 / 180) * 3.14159265358979) * 5,
                    Math.sin(
                        ((this._SafeStr_795 * 10) / 180) * 3.14159265358979
                    ) * 2
                )
            );
    ...
    private changeRotation(): void {
        ...
        if (this._rotation === 0) {
            ...
            const intersection = RoomGeometry.getIntersectionVector(
                location,
                directionAxis,
                new Vector3d(0, 0, 0),
                new Vector3d(0, 0, 1)
            ...
            if (intersection !== null) {
                this._rotationOrigin = new Vector3d(
                    intersection.x,
                    intersection.y,
                    intersection.z
                );
                this._rotationRodLength = Vector3d.dif(
                    intersection,
                    location
                ).length;
                this._rotation = 1;
        ...
        geometry.direction = this._effectDirection;
        geometry.setDepthVector(
            new Vector3d(this._effectDirection.x, this._effectDirection.y, 5)
        );
    }
    ...
    public moveLeft(): void {
        ...
        const geometry = this.geometry as RoomGeometry;
        const direction =
            ((geometry.direction.x - 90) / 180) * 3.14159265358979;

        geometry.location = Vector3d.sum(
            geometry.location,
            new Vector3d(
                Math.cos(direction) * Math.sqrt(2),
                Math.sin(direction) * Math.sqrt(2)
            )
        );
    }
    ...
    public moveDown(): void {
        ...
        const geometry = this.geometry as RoomGeometry;
        const direction =
            ((geometry.direction.x + 180) / 180) * 3.14159265358979;
    ...
    public set screenOffsetX(x: number) {
        ...
        this._mouseLocation.x =
            this._mouseLocation.x - (x - this._screenOffsetX);
    ...
    public set screenOffsetY(y: number) {
        ...
        this._mouseLocation.y =
            this._mouseLocation.y - (y - this._screenOffsetY);
