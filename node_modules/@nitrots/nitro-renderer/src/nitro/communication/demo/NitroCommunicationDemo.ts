import { InventoryMessageEvent } from '@nitrots/nitro-renderer/src/nitro';
...
export class NitroCommunicationDemo extends NitroManager {
    ...
    protected onInit(): void {
        ...
        console.log("[NitroCommunicationDemo] onit")
        if (connection) {
        ...
        this._communication.registerMessageEvent(
            new ClientPingEvent(this.onClientPingEvent.bind(this))
        );
        this._communication.registerMessageEvent(
            new AuthenticatedEvent(this.onAuthenticatedEvent.bind(this))
        );
     //   this._communication.registerMessageEvent(
    ...
    //     console.log('Received inventory items:', parse=.items);
    //     // Optionally you can emit this to a context or global store if needed
    // }
    ...
    private onConnectionOpenedEvent(event: Event): void {
        ...
        this.dispatchCommunicationDemoEvent(
            NitroCommunicationDemoEvent.CONNECTION_ESTABLISHED,
            connection
        );

        if (
            Nitro.instance.getConfiguration<boolean>(
                'system.pong.manually',
                false
            )
        )
            this.startPonging();
    ...
    private tryAuthentication(connection: IConnection): void {
        if (!connection || !this.getSSO()) {
            if (!this.getSSO()) {
                this.logger.error(
                    'Login without an SSO ticket is not supported'
                );
            }
        ...
        connection.send(
            new SSOTicketMessageComposer(this.getSSO(), Nitro.instance.time)
        );
    }
    ...
    private completeHandshake(connection: IConnection): void {
        this.dispatchCommunicationDemoEvent(
            NitroCommunicationDemoEvent.CONNECTION_HANDSHAKED,
            connection
        );
    ...
    private startPonging(): void {
        ...
        this._pongInterval = setInterval(
            this.sendPong,
            Nitro.instance.getConfiguration<number>(
                'system.pong.interval.ms',
                20000
            )
        );
    }
    ...
    private dispatchCommunicationDemoEvent(
        ...
        Nitro.instance.events.dispatchEvent(
            new NitroCommunicationDemoEvent(type, connection)
        );
    }
