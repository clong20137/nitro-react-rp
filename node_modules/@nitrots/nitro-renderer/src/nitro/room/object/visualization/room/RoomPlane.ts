...
export class RoomPlane implements IRoomPlane {
    ...
    constructor(
        ...
        textureMaxX: number = 0,
        textureMaxY: number = 0
    ) {
    ...
    public get bitmapData(): Texture<Resource> {
        ...
        let texture: RenderTexture = RoomVisualization.getTextureCache(
            this._bitmapData
        );
    ...
    public copyBitmapData(k: Texture<Resource>): Texture<Resource> {
        ...
        if (
            this._bitmapData.width !== k.width ||
            this._bitmapData.height !== k.height
        )
            return null;
    ...
    private needsNewTexture(k: IRoomGeometry, _arg_2: number): boolean {
        ...
        if (!planeBitmap) {
            planeBitmap = this._textures.get(
                this.getTextureIdentifier(k.scale)
            );
        }
        ...
        if (
            this._canBeVisible &&
            (!planeBitmap ||
                (planeBitmap.timeStamp >= 0 &&
                    _arg_2 > planeBitmap.timeStamp) ||
                this._maskChanged)
        )
            return true;
    ...
    private getTexture(k: IRoomGeometry, _arg_2: number): Graphics {
        ...
        if (this.needsNewTexture(k, _arg_2)) {
            ...
            if (this._rasterizer) {
                _local_3 = this._rasterizer.render(
                    _local_8,
                    this._id,
                    _local_5,
                    _local_6,
    ...
    private resolveMasks(geom: IRoomGeometry): PlaneDrawingData | null {
        if (!this._useMask) return null;

        const out = new PlaneDrawingData();

        for (let i = 0; i < this._bitmapMasks.length; i++) {
            ...
            const assetName = planeMask.getAssetName(geom.scale);
            if (!assetName) continue;

            const pos = geom.getCoordinatePosition(this._normal);
            const asset = planeMask.getGraphicAsset(geom.scale, pos);
            if (!asset) continue;

            const px =
                this._maskBitmapData.width *
                (1 - mask.leftSideLoc / this._leftSide.length);
            const py =
                this._maskBitmapData.height *
                (1 - mask.rightSideLoc / this._rightSide.length);

            out.addMask(
                assetName,
                new Point(px + asset.offsetX, py + asset.offsetY),
                asset.flipH,
                asset.flipV
            );
        }
    ...
    private screenWidth(k: IRoomGeometry): number {
        ...
        return Math.round(
            this._leftSide.length * Math.abs(_local_2.x - _local_3.x)
        );
    }
            ...
            while (i < layers.length) {
                ...
                if (layer) {
                    if (this._hasTexture && layer.getMaterial()) {
                        const normal = geometry.getCoordinatePosition(
                            this._normal
                        );
                        const cm = layer
                            .getMaterial()
                        ...
                        const data = new PlaneDrawingData(
                            maskData,
                            this._color,
                            cm.isBottomAligned()
                        );
                        ...
                        for (const column of cm.getColumns(
                            this.screenWidth(geometry)
                        )) {
    ...
    public update(geometry: IRoomGeometry, timeSinceStartMs: number): boolean {
        ...
        if (geometryChanged) {
            ...
            while (i < this._secondaryNormals.length) {
                cosAngle = Vector3d.cosAngle(
                    geometry.directionAxis,
                    this._secondaryNormals[i]
                );
            ...
            if (this._type === RoomPlane.TYPE_FLOOR) {
                relativeDepth =
                    relativeDepth -
                    (this._location.z +
                        Math.min(0, this._leftSide.z, this._rightSide.z)) *
                        8;
            }
        ...
        if (
            geometryChanged ||
            this.needsNewTexture(geometry, timeSinceStartMs)
        ) {
            if (
                !this._bitmapData ||
    ...
    private updateCorners(k: IRoomGeometry): void {
        ...
        this._cornerB.assign(
            k.getScreenPosition(Vector3d.sum(this._location, this._rightSide))
        );
        this._cornerC.assign(
            k.getScreenPosition(
                Vector3d.sum(
                    Vector3d.sum(this._location, this._leftSide),
                    this._rightSide
                )
            )
        );
        this._cornerD.assign(
            k.getScreenPosition(Vector3d.sum(this._location, this._leftSide))
        );
        ...
        const _local_2: number = Math.min(
            this._cornerA.x,
            this._cornerB.x,
            this._cornerC.x,
            this._cornerD.x
        );
        let _local_3: number = Math.max(
            this._cornerA.x,
            this._cornerB.x,
            this._cornerC.x,
            this._cornerD.x
        );
        const _local_4: number = Math.min(
            this._cornerA.y,
            this._cornerB.y,
            this._cornerC.y,
            this._cornerD.y
        );
        let _local_5: number = Math.max(
            this._cornerA.y,
            this._cornerB.y,
            this._cornerC.y,
            this._cornerD.y
    ...
    private renderTexture(k: IRoomGeometry, _arg_2: Graphics): void {
        ...
        if (
            this._type == RoomPlane.TYPE_WALL ||
            this._type == RoomPlane.TYPE_LANDSCAPE
        ) {
            if (Math.abs(_local_5 - _arg_2.width) <= 1) _local_5 = _arg_2.width;
            if (Math.abs(_local_6 - _arg_2.height) <= 1)
                _local_6 = _arg_2.height; // <-- was width
    ...
    public resetBitmapMasks(): void {
        if (this._disposed || !this._useMask || !this._bitmapMasks.length)
            return;
    ...
    public addBitmapMask(k: string, _arg_2: number, _arg_3: number): boolean {
        ...
        while (_local_5 < this._bitmapMasks.length) {
            ...
            if (mask) {
                if (
                    mask.type === k &&
                    mask.leftSideLoc === _arg_2 &&
                    mask.rightSideLoc === _arg_3
                )
                    return false;
            }
    ...
    public addRectangleMask(
        ...
        if (this._useMask) {
            for (const mask of this._rectangleMasks) {
                ...
                if (
                    mask.leftSideLoc === k &&
                    mask.rightSideLoc === _arg_2 &&
                    mask.leftSideLength === _arg_3 &&
                    mask.rightSideLength === _arg_4
                )
                    return false;
            }

            const _local_5 = new RoomPlaneRectangleMask(
                k,
                _arg_2,
                _arg_3,
                _arg_4
        ...
        if (this._bitmapMasks.length === this._bitmapMasksOld.length) {
            for (const mask of this._bitmapMasks) {
                ...
                for (const plane of this._bitmapMasksOld) {
                    ...
                    if (
                        plane.type === mask.type &&
                        plane.leftSideLoc === mask.leftSideLoc &&
                        plane.rightSideLoc === mask.rightSideLoc
                    ) {
                        _local_6 = true;
    ...
    private updateMask(texture: Graphics, geometry: IRoomGeometry): void {
        ...
        if (
            !this._useMask ||
            (!this._bitmapMasks.length &&
                !this._rectangleMasks.length &&
                !this._maskChanged) ||
            !this._maskManager
        )
            return;
        ...
        if (
            ...
            this._maskBitmapData.width !== width ||
            this._maskBitmapData.height !== height
        ) {
            if (this._maskBitmapData) {
                this._maskBitmapData.destroy();
                this._maskBitmapData = null;
        ...
        if (this._maskChanged) {
            ...
            while (i < this._bitmapMasks.length) {
                ...
                if (mask) {
                    ...
                    posX =
                        this._maskBitmapData.width -
                        (this._maskBitmapData.width * mask.leftSideLoc) /
                            this._leftSide.length;
                    posY =
                        this._maskBitmapData.height -
                        (this._maskBitmapData.height * mask.rightSideLoc) /
                            this._rightSide.length;

                    this._maskManager.updateMask(
                        this._maskBitmapData,
                        type,
                        geometry.scale,
                        normal,
                    ...
                    this._bitmapMasksOld.push(
                        new RoomPlaneBitmapMask(
                            type,
                            mask.leftSideLoc,
                            mask.rightSideLoc
                        )
            ...
            while (i < this._rectangleMasks.length) {
                ...
                if (rectMask) {
                    posX =
                        this._maskBitmapData.width -
                        (this._maskBitmapData.width * rectMask.leftSideLoc) /
                            this._leftSide.length;
                    posY =
                        this._maskBitmapData.height -
                        (this._maskBitmapData.height * rectMask.rightSideLoc) /
                            this._rightSide.length;

                    const wd =
                        (this._maskBitmapData.width * rectMask.leftSideLength) /
                        this._leftSide.length;
                    const ht =
                        (this._maskBitmapData.height *
                            rectMask.rightSideLength) /
                        this._rightSide.length;
                    ...
                    this._rectangleMasksOld.push(
                        new RoomPlaneRectangleMask(
                            rectMask.leftSideLoc, // was leftSideLength
                            rectMask.rightSideLoc,
                            rectMask.leftSideLength,
                            rectMask.rightSideLength
                        )
                    );
                }
    ...
    private combineTextureMask(texture: Graphics, mask: Graphics): void {
        ...
        const textureImageData = textureCtx.getImageData(
            0,
            0,
            textureCanvas.width,
