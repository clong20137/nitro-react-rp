...
export class RoomObjectEventHandler
    extends Disposable
    implements IRoomCanvasMouseListener
{
    ...
    constructor(roomEngine: IRoomEngineServices) {
        ...
        this._roomEngine.events.addEventListener(
    ...
    private onRoomEngineObjectEvent(event: RoomEngineObjectEvent): void {
        ...
        if (
            ...
            selectedData.id === event.objectId
        ) {
            ...
            if (roomObject && roomObject.model) {
                if (selectedData.category === RoomObjectCategory.FLOOR) {
                    const allowedDirections = roomObject.model.getValue<
                        number[]
                    >(RoomObjectVariable.FURNITURE_ALLOWED_DIRECTIONS);
                    ...
                    if (allowedDirections && allowedDirections.length) {
                        ...
                        selectedData = this.getSelectedRoomObjectData(
                            event.roomId
                        );
    ...
    public processRoomCanvasMouseEvent(
        event: RoomSpriteMouseEvent,
        object: IRoomObject,
        ...
        if (
            category !== RoomObjectCategory.ROOM &&
            (!this._roomEngine.isPlayingGame() ||
    ...
    public processRoomObjectPlacement(
        ...
        if (this._roomEngine) {
            this._roomEngine.setObjectMoverIconSprite(
                typeId,
                category,
                false,
                extra,
    ...
    public handleRoomObjectEvent(event: RoomObjectEvent, roomId: number): void {
        ...
        switch (event.type) {
            ...
            case RoomObjectStateChangedEvent.STATE_RANDOM:
                this.onRoomObjectStateChangedEvent(
                    event as RoomObjectStateChangedEvent,
                    roomId
                );
                return;
            case RoomObjectDimmerStateUpdateEvent.DIMMER_STATE:
                this.onRoomObjectDimmerStateUpdateEvent(
                    event as RoomObjectDimmerStateUpdateEvent,
                    roomId
                );
                return;
            case RoomObjectMoveEvent.POSITION_CHANGED:
            case RoomObjectMoveEvent.OBJECT_REMOVED:
                this.handleSelectedObjectRemove(
                    event as RoomObjectMoveEvent,
                    roomId
                );
                return;
            ...
            case RoomObjectWidgetRequestEvent.YOUTUBE:
                this.onRoomObjectWidgetRequestEvent(
                    event as RoomObjectWidgetRequestEvent,
                    roomId
                );
                return;
            ...
            case RoomObjectFurnitureActionEvent.ENTER_ONEWAYDOOR:
                this.onRoomObjectFurnitureActionEvent(
                    event as RoomObjectFurnitureActionEvent,
                    roomId
                );
                return;
            ...
            case RoomObjectFloorHoleEvent.REMOVE_HOLE:
                this.onRoomObjectFloorHoleEvent(
                    event as RoomObjectFloorHoleEvent,
                    roomId
                );
                return;
            ...
            case RoomObjectRoomAdEvent.ROOM_AD_LOAD_IMAGE:
                this.onRoomObjectRoomAdEvent(
                    event as RoomObjectRoomAdEvent,
                    roomId
                );
                return;
            case RoomObjectBadgeAssetEvent.LOAD_BADGE:
                this.onRoomObjectBadgeAssetEvent(
                    event as RoomObjectBadgeAssetEvent,
                    roomId
                );
                return;
            ...
            case RoomObjectPlaySoundIdEvent.PLAY_SOUND_AT_PITCH:
                this.handleRoomObjectPlaySoundEvent(
                    event as RoomObjectPlaySoundIdEvent,
                    roomId
                );
                return;
            ...
            case RoomObjectSamplePlaybackEvent.CHANGE_PITCH:
                this.handleRoomObjectSamplePlaybackEvent(
                    event as RoomObjectSamplePlaybackEvent,
                    roomId
                );
                return;
            case RoomObjectHSLColorEnableEvent.ROOM_BACKGROUND_COLOR:
                this.onHSLColorEnableEvent(
                    event as RoomObjectHSLColorEnableEvent,
                    roomId
                );
                return;
            case RoomObjectDataRequestEvent.RODRE_CURRENT_USER_ID:
            case RoomObjectDataRequestEvent.RODRE_URL_PREFIX:
                this.onRoomObjectDataRequestEvent(
                    event as RoomObjectDataRequestEvent,
                    roomId
                );
                return;
            default:
    ...
    private handleRoomObjectMouseClickEvent(
        ...
        // CLICK-THROUGH: if enabled and we click a USER avatar,
        // treat it as clicking the tile and just walk there.
        const ctEnabled = (window as any).__ctEnabled === true;

        if (ctEnabled && event.objectType === RoomObjectUserType.USER) {
            // If we actually got a tile mouse event, use its tile coords
            if (event instanceof RoomObjectTileMouseEvent) {
                this.onRoomObjectTileMouseEvent(roomId, event);
            } else {
                // Fallback: walk to the avatar's current tile
                const avatarObj = this._roomEngine.getRoomObject(
                    roomId,
                    event.objectId,
                ...
                if (avatarObj) {
                    const loc = avatarObj.getLocation();
                    this.sendWalkUpdate(~~loc.x, ~~loc.y);
                }
            }

            // Do NOT run the normal avatar click logic
            return;
        }
        ...
        const category = this._roomEngine.getRoomObjectCategoryForType(
            event.objectType
        );
                    ...
                    if (category === RoomObjectCategory.UNIT) {
                        if (
                            ...
                            this.modifyRoomObject(
                                ...
                                event.objectId,
                                category,
                                RoomObjectOperationType.OBJECT_PICKUP_BOT
                            );
                        } else if (
                            event.ctrlKey &&
                            !event.altKey &&
                            !event.shiftKey &&
                            event.objectType ===
                                RoomObjectUserType.MONSTER_PLANT
                        ) {
                            this.modifyRoomObject(
                                roomId,
                                event.objectId,
                                category,
                                RoomObjectOperationType.OBJECT_PICKUP_PET
                            );
                        } else if (
                            !event.ctrlKey &&
                            !event.altKey &&
        ...
        if (category === RoomObjectCategory.ROOM) {
            const _local_15 = this.getMouseEventId(
                RoomObjectCategory.MINIMUM,
                MouseEventType.MOUSE_CLICK
            );
            const _local_16 = this.getMouseEventId(
                RoomObjectCategory.UNIT,
                MouseEventType.MOUSE_CLICK
            );

            if (
                _local_15 !== event.eventId &&
                _local_16 !== event.eventId &&
                !didMove
            ) {
    ...
    private handleRoomObjectMouseMoveEvent(
        ...
        if (this._roomEngine) {
            ...
            if (roomCursor && roomCursor.logic) {
                ...
                if (event instanceof RoomObjectTileMouseEvent) {
                    ...
                    if (this._whereYouClickIsWhereYouGo) {
                        newEvent = this.handleMouseOverObject(
                            category,
                            roomId,
                            event
                        );
                    ...
                    newEvent = new ObjectTileCursorUpdateMessage(
                        null,
                        0,
                        false,
                        event.eventId
        ...
        switch (operation) {
            case RoomObjectOperationType.OBJECT_MOVE:
                if (category === RoomObjectCategory.ROOM)
                    this.handleObjectMove(event, roomId);
                ...
                if (
                    ...
                    if (
                        ...
                        if (this._roomEngine.events)
                            this._roomEngine.events.dispatchEvent(
                                new RoomEngineObjectEvent(
                                    RoomEngineObjectEvent.REQUEST_MANIPULATION,
                                    roomId,
                                    event.objectId,
                                    category
                                )
    ...
    private handleRoomObjectMouseLeaveEvent(
        ...
        if (category !== RoomObjectCategory.ROOM) {
            if (category === RoomObjectCategory.UNIT) {
                ...
                if (cursor)
                    cursor.processUpdateMessage(
                        new ObjectDataUpdateMessage(0, null)
                    );
            }
        }
    ...
    private onRoomObjectStateChangedEvent(
        event: RoomObjectStateChangedEvent,
        roomId: number
    ): void {
        if (!event) return;
    ...
    private onRoomObjectDimmerStateUpdateEvent(
        event: RoomObjectDimmerStateUpdateEvent,
        roomId: number
    ): void {
        if (!event) return;
        ...
        switch (event.type) {
            case RoomObjectMoveEvent.POSITION_CHANGED: {
                ...
                const objectCategory =
                    this._roomEngine.getRoomObjectCategoryForType(objectType);
                const object = this._roomEngine.getRoomObject(
                    roomId,
                    objectId,
                    objectCategory
                );
                const selectionArrow =
                    this._roomEngine.getRoomObjectSelectionArrow(roomId);

                if (object && selectionArrow && selectionArrow.logic) {
                    const location = object.getLocation();

                    selectionArrow.logic.processUpdateMessage(
                        new RoomObjectUpdateMessage(location, null)
                    );
                }
                return;
            }
    ...
    private onRoomObjectWidgetRequestEvent(
        event: RoomObjectWidgetRequestEvent,
        roomId: number
    ): void {
        if (!event || !this._roomEngine) return;
        ...
        switch (event.type) {
            ...
            case RoomObjectWidgetRequestEvent.OPEN_FURNI_CONTEXT_MENU:
                eventDispatcher.dispatchEvent(
                    new RoomEngineTriggerWidgetEvent(
                        ...
                        objectCategory,
                        (
                            event.object as IRoomObjectController
                        ).logic.contextMenu
                    )
                );
                return;
            ...
            case RoomObjectWidgetRequestEvent.PLACEHOLDER:
                eventDispatcher.dispatchEvent(
                    new RoomEngineTriggerWidgetEvent(
                        RoomEngineTriggerWidgetEvent.REQUEST_PLACEHOLDER,
                        roomId,
                        objectId,
                        objectCategory
                    )
            ...
            case RoomObjectWidgetRequestEvent.GUILD_FURNI_CONTEXT_MENU:
                this._roomEngine.connection.send(
                    new FurnitureGroupInfoComposer(
                        event.objectId,
                        event.object.model.getValue<number>(
                            RoomObjectVariable.FURNITURE_GUILD_CUSTOMIZED_GUILD_ID
                        )
                    )
                );
                return;
            ...
            case RoomObjectWidgetRequestEvent.ACHIEVEMENT_RESOLUTION_OPEN:
                this._roomEngine.connection.send(
                    new GetResolutionAchievementsMessageComposer(
                        event.objectId,
                        0
                    )
                );
            ...
            case RoomObjectWidgetRequestEvent.FRIEND_FURNITURE_CONFIRM:
                eventDispatcher.dispatchEvent(
                    new RoomEngineTriggerWidgetEvent(
                        RoomEngineTriggerWidgetEvent.REQUEST_FRIEND_FURNITURE_CONFIRM,
                        roomId,
                        objectId,
                        objectCategory
                    )
            ...
            case RoomObjectWidgetRequestEvent.YOUTUBE:
                eventDispatcher.dispatchEvent(
                    new RoomEngineTriggerWidgetEvent(
                        RoomEngineTriggerWidgetEvent.REQUEST_YOUTUBE,
                        roomId,
                        objectId,
                        objectCategory
                    )
    ...
    private onRoomObjectFurnitureActionEvent(
        event: RoomObjectFurnitureActionEvent,
        roomId: number
    ): void {
        if (!event) return;
    ...
    private handleObjectSoundMachineEvent(
        event: RoomObjectEvent,
        roomId: number
    ): void {
        if (!event) return;

        const objectCategory = this._roomEngine.getRoomObjectCategoryForType(
            event.objectType
        );
        const selectedData = this.getSelectedRoomObjectData(roomId);
        ...
        if (selectedData) {
            if (
                selectedData.category === objectCategory &&
                selectedData.id === event.objectId
            ) {
                if (
                    selectedData.operation ===
        ...
        switch (event.type) {
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_INIT:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.SOUND_MACHINE_INIT,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
            ...
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_START:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.SOUND_MACHINE_SWITCHED_ON,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
            ...
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_STOP:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.SOUND_MACHINE_SWITCHED_OFF,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
            ...
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_DISPOSE:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.SOUND_MACHINE_DISPOSE,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
    ...
    private handleObjectJukeboxEvent(
        ...
        switch (event.type) {
            case RoomObjectFurnitureActionEvent.JUKEBOX_INIT:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.JUKEBOX_INIT,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
            ...
            case RoomObjectFurnitureActionEvent.JUKEBOX_START:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.JUKEBOX_SWITCHED_ON,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
            ...
            case RoomObjectFurnitureActionEvent.JUKEBOX_MACHINE_STOP:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.JUKEBOX_SWITCHED_OFF,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
            ...
            case RoomObjectFurnitureActionEvent.JUKEBOX_DISPOSE:
                this._roomEngine.events.dispatchEvent(
                    new RoomObjectSoundMachineEvent(
                        RoomObjectSoundMachineEvent.JUKEBOX_DISPOSE,
                        roomId,
                        event.objectId,
                        objectCategory
                    )
    ...
    private onRoomObjectFloorHoleEvent(
        event: RoomObjectFloorHoleEvent,
        roomId: number
    ): void {
        if (!event) return;
        ...
        switch (event.type) {
            case RoomObjectFloorHoleEvent.ADD_HOLE:
                this._roomEngine.addRoomInstanceFloorHole(
                    roomId,
                    event.objectId
                );
                return;
            case RoomObjectFloorHoleEvent.REMOVE_HOLE:
                this._roomEngine.removeRoomInstanceFloorHole(
                    roomId,
                    event.objectId
                );
                return;
            ...
            case RoomObjectRoomAdEvent.ROOM_AD_FURNI_DOUBLE_CLICK:
                if (event.clickUrl && event.clickUrl.length > 0) {
                    ...
                    if (event.clickUrl.indexOf(catalogPage) === 0) {
                        Nitro.instance.createLinkEvent(
                            event.clickUrl.substr(catalogPage.length)
                        );
                    }
                }
        ...
        if (eventType)
            this._roomEngine.events.dispatchEvent(
                new RoomEngineObjectEvent(
                    ...
                    roomId,
                    event.objectId,
                    this._roomEngine.getRoomObjectCategoryForType(
                        event.objectType
                    )
                )
            );
    ...
    private onRoomObjectBadgeAssetEvent(
        ...
        switch (event.type) {
            case RoomObjectBadgeAssetEvent.LOAD_BADGE: {
                ...
                this._roomEngine.loadRoomObjectBadgeImage(
                    roomId,
                    objectId,
                    objectCategory,
                    event.badgeId,
    ...
    private handleMousePointer(
        ...
        this._roomEngine.updateMousePointer(
            event.type,
            event.objectId,
            event.objectType
        );
    ...
    private handleRoomObjectPlaySoundEvent(
        event: RoomObjectPlaySoundIdEvent,
        roomId: number
    ): void {
        const objectCategory = this._roomEngine.getRoomObjectCategoryForType(
            event.objectType
    ...
    private handleRoomObjectSamplePlaybackEvent(
        event: RoomObjectSamplePlaybackEvent,
        roomId: number
    ): void {
        if (!event) return;
        ...
        switch (event.type) {
            ...
            case RoomObjectSamplePlaybackEvent.PLAY_SAMPLE:
                this._roomEngine.events.dispatchEvent(
                    new RoomEngineSamplePlaybackEvent(
                        RoomEngineSamplePlaybackEvent.PLAY_SAMPLE,
                        roomId,
                        event.objectId,
                        objectCategory,
                        event.sampleId,
            ...
            case RoomObjectSamplePlaybackEvent.CHANGE_PITCH:
                this._roomEngine.events.dispatchEvent(
                    new RoomEngineSamplePlaybackEvent(
                        RoomEngineSamplePlaybackEvent.CHANGE_PITCH,
                        roomId,
                        event.objectId,
                        objectCategory,
                        event.sampleId,
    ...
    private onHSLColorEnableEvent(
        event: RoomObjectHSLColorEnableEvent,
        roomId: number
    ): void {
        if (!event || !this._roomEngine) return;
    ...
    private onRoomObjectDataRequestEvent(
        ...
        switch (event.type) {
            case RoomObjectDataRequestEvent.RODRE_CURRENT_USER_ID:
                event.object.model.setValue(
                    RoomObjectVariable.SESSION_CURRENT_USER_ID,
                    this._roomEngine.sessionDataManager.userId
                );
                return;
            case RoomObjectDataRequestEvent.RODRE_URL_PREFIX:
                event.object.model.setValue(
                    RoomObjectVariable.SESSION_URL_PREFIX,
                    Nitro.instance.getConfiguration('url.prefix')
                );
                return;
        }
    }
    ...
    private onRoomObjectTileMouseEvent(
        roomId: number,
        event: RoomObjectTileMouseEvent
    ): void {
        if (
            !this._roomEngine ||
            this._roomEngine.isDecorating ||
            !this._roomEngine.roomSessionManager
        )
            return;
    ...
    private handleObjectMove(
        ...
        if (
            selectedData.category === RoomObjectCategory.FLOOR ||
            selectedData.category === RoomObjectCategory.UNIT
        ) {
            const stackingHeightMap =
                this._roomEngine.getFurnitureStackingHeightMap(roomId);
            ...
            this._roomEngine.updateRoomObjectMask(
                roomId,
                selectedData.id,
                _local_6
            );
    ...
    private handleObjectPlace(
        ...
        let roomObject = this._roomEngine.getRoomObject(
            roomId,
            selectedData.id,
            selectedData.category
        );
        ...
        if (!roomObject) {
            if (event instanceof RoomObjectTileMouseEvent) {
                if (selectedData.category === RoomObjectCategory.FLOOR) {
                    ...
                    this._roomEngine.addRoomObjectUser(
                        roomId,
                        selectedData.id,
                        new Vector3d(),
                        new Vector3d(180),
                ...
                if (selectedData.category === RoomObjectCategory.WALL) {
                    this._roomEngine.addFurnitureWall(
                        roomId,
                        selectedData.id,
                        selectedData.typeId,
                        selectedData.loc,
            ...
            roomObject = this._roomEngine.getRoomObject(
                roomId,
                selectedData.id,
                selectedData.category
            );
        ...
        if (roomObject) {
            ...
            if (selectedData.category === RoomObjectCategory.FLOOR) {
                if (
                    ...
                    this._roomEngine.removeRoomObjectFloor(
                        roomId,
                        selectedData.id
                    );
                ...
                if (!_local_12) {
                    this._roomEngine.removeRoomObjectWall(
                        roomId,
                        selectedData.id
                    );
                }
                ...
                if (
                    !(
                        event instanceof RoomObjectTileMouseEvent &&
                        this.handleUserPlace(
                            roomObject,
                            Math.trunc(event.tileX + 0.5),
                            Math.trunc(event.tileY + 0.5),
                            this._roomEngine.getLegacyWallGeometry(roomId)
                        )
                    )
                ) {
                    this._roomEngine.removeRoomObjectUser(
                        roomId,
                        selectedData.id
                    );
    ...
    private handleFurnitureMove(
        ...
        selectedObjectData: SelectedRoomObjectData,
        x: number,
        y: number,
        stackingHeightMap: FurnitureStackingHeightMap
    ): boolean {
        ...
        let _local_9 = this.validateFurnitureLocation(
            roomObject,
            _local_7,
            selectedObjectData.loc,
            selectedObjectData.dir,
            stackingHeightMap
        );
        ...
        if (!_local_9) {
            ...
            _local_9 = this.validateFurnitureLocation(
                roomObject,
                _local_7,
                selectedObjectData.loc,
                selectedObjectData.dir,
    ...
    private handleWallItemMove(
        ...
        const _local_10 = this.validateWallItemLocation(
            k,
            _arg_3,
            _arg_4,
            _arg_5,
    ...
    private validateFurnitureLocation(
        ...
        _arg_3: IVector3D,
        _arg_4: IVector3D,
        _arg_5: FurnitureStackingHeightMap
    ): Vector3d {
        ...
        let sizeX = k.model.getValue<number>(
            RoomObjectVariable.FURNITURE_SIZE_X
        );
        let sizeY = k.model.getValue<number>(
        ...
        if (_arg_5 && _arg_2) {
            const stackable =
                k.model.getValue<number>(
                    RoomObjectVariable.FURNITURE_ALWAYS_STACKABLE
                ) === 1;

            if (
                _arg_5.validateLocation(
                    _arg_2.x,
                    _arg_2.y,
                    sizeX,
                    sizeY,
                ...
                return new Vector3d(
                    _arg_2.x,
                    _arg_2.y,
                    _arg_5.getTileHeight(_arg_2.x, _arg_2.y)
                );
            }
    ...
    private validateWallItemLocation(
        ...
        const _local_8 = k.model.getValue<number>(
            RoomObjectVariable.FURNITURE_SIZE_X
        );
        const _local_9 = k.model.getValue<number>(
            RoomObjectVariable.FURNITURE_SIZE_Z
        );
        const _local_10 = k.model.getValue<number>(
            RoomObjectVariable.FURNITURE_CENTER_Z
        );
    ...
    private useObject(
        ...
        switch (action) {
            case RoomObjectFurnitureActionEvent.DICE_ACTIVATE:
                this._roomEngine.connection.send(
                    new FurnitureDiceActivateComposer(objectId)
                );
                return;
            case RoomObjectFurnitureActionEvent.DICE_OFF:
                this._roomEngine.connection.send(
                    new FurnitureDiceDeactivateComposer(objectId)
                );
                return;
            case RoomObjectFurnitureActionEvent.USE_HABBOWHEEL:
                this._roomEngine.connection.send(
                    new FurnitureColorWheelComposer(objectId)
                );
                return;
            case RoomObjectFurnitureActionEvent.STICKIE:
                this._roomEngine.connection.send(
                    new GetItemDataComposer(objectId)
                );
                return;
            case RoomObjectFurnitureActionEvent.ENTER_ONEWAYDOOR:
                this._roomEngine.connection.send(
                    new FurnitureOneWayDoorComposer(objectId)
                );
                return;
        }
    }
    ...
    private changeRoomObjectState(
        ...
        if (category === RoomObjectCategory.FLOOR) {
            if (!isRandom) {
                this._roomEngine.connection.send(
                    new FurnitureMultiStateComposer(objectId, state)
                );
            } else {
                this._roomEngine.connection.send(
                    new FurnitureRandomStateComposer(objectId, state)
                );
            }
        } else if (category === RoomObjectCategory.WALL) {
            this._roomEngine.connection.send(
                new FurnitureWallMultiStateComposer(objectId, state)
            );
        }
    ...
    private handleMoveTargetFurni(
        ...
        const _local_3 = this._roomEngine.getRoomObject(
            k,
            _arg_2.objectId,
            RoomObjectCategory.FLOOR
        );
    ...
    private getActiveSurfaceLocation(
        ...
        const furniData =
            this._roomEngine.sessionDataManager.getFloorItemDataByName(k.type);
        ...
        const sizeZ = model.getValue<number>(
            RoomObjectVariable.FURNITURE_SIZE_Z
        );
        ...
        const renderingCanvas =
            this._roomEngine.getActiveRoomInstanceRenderingCanvas();
        ...
        const _local_14 =
            (scale / 2 + _arg_2.spriteOffsetX + _arg_2.localX) / (scale / 4);
        const _local_15 =
            (_arg_2.spriteOffsetY +
                _arg_2.localY +
                ((sizeZ - _local_13) * scale) / 2) /
            (scale / 4);
    ...
    private handleMouseOverTile(
        k: RoomObjectTileMouseEvent,
        roomId: number
    ): ObjectTileCursorUpdateMessage {
        if (this._whereYouClickIsWhereYouGo) {
            return new ObjectTileCursorUpdateMessage(
                new Vector3d(k.tileXAsInt, k.tileYAsInt, k.tileZAsInt),
                0,
                true,
                k.eventId
            );
        ...
        if (roomObject && roomObject.visualization) {
            ...
            if (_local_7) {
                ...
                if (_local_8) {
                    const _local_9 = _local_8.getObjectIntTile(
                        _local_4,
                        _local_5
                    );
                    const _local_10 =
                    ...
                    if (_local_10) {
                        if (
                            _local_9 &&
                            _local_9.model &&
                            _local_9.model.getValue<number>(
                                RoomObjectVariable.FURNITURE_IS_VARIABLE_HEIGHT
                            ) > 0
                        ) {
                            const _local_11 = _local_10.getTileHeight(
                                _local_4,
                                _local_5
                            );
    ...
    private placeObject(
        ...
        if (this._roomEngine && this._roomEngine.connection) {
            ...
            if (roomObject) {
                ...
                if (
                    ...
                    if (wallGeometry)
                        wallLocation = wallGeometry.getOldLocationString(
                            location,
                            direction
                        );
                }
                ...
                if (objectId < 0 && category === RoomObjectCategory.UNIT)
                    objectId = objectId * -1;
                ...
                if (
                    ...
                    if (category === RoomObjectCategory.UNIT) {
                        if (selectedData.typeId === RoomObjectType.PET) {
                            ...
                            selectedData.typeId === RoomObjectType.RENTABLE_BOT
                        ) {
                            this._roomEngine.connection.send(
                                new BotPlaceComposer(
                                    objectId,
                                    Math.trunc(x),
                                    Math.trunc(y)
                                )
                        ...
                        roomObject.model.getValue<string>(
                            RoomObjectVariable.FURNITURE_IS_STICKIE
                        ) !== undefined
                    ) {
                        this._roomEngine.connection.send(
                            new FurniturePostItPlaceComposer(
                                objectId,
                                wallLocation
                            )
                        );
        ...
        this._roomEngine.setPlacedRoomObjectData(
            roomId,
        ...
        if (this._roomEngine && this._roomEngine.events) {
            const placedInRoom =
                roomObject && roomObject.id === selectedData.id;
        ...
        switch (operation) {
            ...
            case RoomObjectOperationType.OBJECT_ROTATE_NEGATIVE:
                if (this._roomEngine.connection) {
                    ...
                    if (
                        operation ==
                        RoomObjectOperationType.OBJECT_ROTATE_NEGATIVE
                    ) {
                        direction = this.getValidRoomObjectDirection(
                            roomObject,
                            false
                        );
                    } else {
                    ...
                    if (
                        this.isValidLocation(
                            ...
                            this._roomEngine.getFurnitureStackingHeightMap(
                                roomId
                            )
                        )
                    ) {
                        ...
                        if (
                            ...
                            const roomSession =
                                this._roomEngine.roomSessionManager.getSession(
                                    roomId
                                );
                            ...
                            this._roomEngine.connection.send(
                                new FurnitureFloorUpdateComposer(
                                    objectId,
                                    x,
                                    y,
                                    direction
            ...
            case RoomObjectOperationType.OBJECT_PICKUP:
                if (this._roomEngine.connection)
                    this._roomEngine.connection.send(
                        new FurniturePickupComposer(category, objectId)
                    );
                break;
            case RoomObjectOperationType.OBJECT_PICKUP_PET:
                if (this._roomEngine.connection) {
                    const session =
                        this._roomEngine.roomSessionManager.getSession(roomId);

                    if (session) {
                        const userData =
                            session.userDataManager.getUserDataByIndex(
                                objectId
                            );
                ...
                if (this._roomEngine.connection) {
                    if (category === RoomObjectCategory.FLOOR) {
                        ...
                        if (wallGeometry) {
                            ...
                            if (location)
                                this._roomEngine.connection.send(
                                    new FurnitureWallUpdateComposer(
                                        objectId,
                                        location
                                    )
                                );
    ...
    public modifyRoomObjectDataWithMap(
        ...
        switch (operation) {
            case RoomObjectOperationType.OBJECT_SAVE_STUFF_DATA:
                if (this._roomEngine.connection) {
                    this._roomEngine.connection.send(
                        new SetObjectDataMessageComposer(objectId, data)
                    );
                }
                break;
        }
    ...
    public modifyWallItemData(
        ...
        this._roomEngine.connection.send(
            new SetItemDataMessageComposer(objectId, colorHex, text)
        );
    ...
    public getValidRoomObjectDirection(
        k: IRoomObjectController,
        _arg_2: boolean
    ): number {
        ...
        if (k.type === RoomObjectUserType.MONSTER_PLANT) {
            allowedDirections = k.model.getValue<number[]>(
                RoomObjectVariable.PET_ALLOWED_DIRECTIONS
            );
        } else {
            allowedDirections = k.model.getValue<number[]>(
                RoomObjectVariable.FURNITURE_ALLOWED_DIRECTIONS
        ...
        if (allowedDirections && allowedDirections.length) {
            ...
            if (_arg_2) _local_6 = (_local_6 + 1) % allowedDirections.length;
                ...
                _local_6 =
                    (_local_6 - 1 + allowedDirections.length) %
    ...
    private isValidLocation(
        ...
        let sizeX = object.model.getValue<number>(
            RoomObjectVariable.FURNITURE_SIZE_X
        );
        let sizeY = object.model.getValue<number>(
            RoomObjectVariable.FURNITURE_SIZE_Y
        );
        ...
        let _local_11 = Math.trunc(
            (Math.trunc(goalDirection.x + 45) % 360) / 90
        );
    ...
    private placeObjectOnUser(
        ...
        const _local_5 = this._roomEngine.getRoomObject(
            roomId,
            objectId,
            category
                    ...
                    if (objectId !== this._selectedObjectId) {
                        ...
                        if (roomObject && roomObject.logic) {
                            roomObject.logic.processUpdateMessage(
                                new ObjectSelectedMessage(true)
                            );
    ...
    public setSelectedAvatar(k: number, _arg_2: number, _arg_3: boolean): void {
        ...
        const _local_5 = this._roomEngine.getRoomObject(
            k,
            this._selectedAvatarId,
            _local_4
        );

        if (_local_5 && _local_5.logic) {
            _local_5.logic.processUpdateMessage(
                new ObjectAvatarSelectedMessage(false)
            );
        ...
        if (_arg_3) {
            ...
            if (_local_5 && _local_5.logic) {
                _local_5.logic.processUpdateMessage(
                    new ObjectAvatarSelectedMessage(true)
                );
                ...
                if (location)
                    this._roomEngine.connection.send(
                        new RoomUnitLookComposer(~~location.x, ~~location.y)
                    );
            }
        }
        ...
        if (selectionArrow && selectionArrow.logic) {
            if (_local_6 && !this._roomEngine.isPlayingGame())
                selectionArrow.logic.processUpdateMessage(
                    new ObjectVisibilityUpdateMessage(
    ...
    private decorateModeMove(event: RoomObjectMouseEvent): boolean {
        return (
            this._roomEngine.isDecorating && !(event.ctrlKey || event.shiftKey)
        );
    }
    ...
    private handleUserPlace(
        ...
        roomObject.setLocation(
            new Vector3d(x, y, wallGeometry.getHeight(x, y))
        );
