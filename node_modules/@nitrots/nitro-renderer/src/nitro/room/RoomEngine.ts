...
import { IBulkAddedFloorItem } from '@nitrots/nitro-renderer/src/nitro/communication/messages/parser/BulkFurnitureFloorAddParser';
        ...
        IDisposable
{
    ...
    private _badgeListenerObjects: Map<
        string,
    ...
    constructor(communication: INitroCommunicationManager) {
        ...
        this.onRoomContentLoaderReadyEvent =
            this.onRoomContentLoaderReadyEvent.bind(this);
    ...
    public onInit(): void {
        ...
        this._communication.connection.addMessageEvent(
            new BulkFurnitureFloorRemoveEvent(
                this.onBulkFurnitureFloorRemoveEvent.bind(this)
            )
        );
        this._communication.connection.addMessageEvent(
            new BulkAddFloorItemsEvent(this.onBulkAddFloorItemsEvent.bind(this))
        );
        ...
        if (this._roomSessionManager) {
            this._roomSessionManager.events.addEventListener(
        ...
        this.events.addEventListener(
            RoomContentLoader.LOADER_READY,
            this.onRoomContentLoaderReadyEvent
        );
    ...
    private onBulkAddFloorItemsEvent(event: BulkAddFloorItemsEvent): void {
        ...
        // Spawn everything first, then refresh the tile map ONCE.
        this.addFloorItemsByWire(items, /*refreshMap=*/ true);
        console.log('BulkItemsAdded');
    }


    public addFloorItemsByWire = (
        items: IBulkAddedFloorItem[],
        refreshMap: boolean = true
    ): void => {
        if (!items || !items.length) return;

        const roomId = this._activeRoomId;
        const inst = this.getRoomInstance(roomId) as RoomInstance;
        if (!inst) return;

        // Clear any “pending add/remove” queues for these ids so nothing fights us
        const instanceData = this.getRoomInstanceData(roomId);
        if (instanceData) {
            for (const it of items)
                instanceData.removePendingFunitureFloor(it.itemId);
        }

        // Refresh stacking/collision ONCE (crucial for perf + removing that half-sec delay)
        if (refreshMap)
            this.refreshTileObjectMap(roomId, 'RoomEngine.addFloorItemsByWire');

        // Canonical batch broadcast for any UI listeners
        window.dispatchEvent(
            new CustomEvent('furni_bulk_add', {
                detail: {
                    items: items.map((i) => ({ itemId: i.itemId })),
                    count: items.length,
                },
            })
        );
    ...
    public addFloorItemByWire = (
        item: IBulkAddedFloorItem,
        refreshMap: boolean = true
    ): void => {
        this.addFloorItemsByWire([item], refreshMap);
    };

    private onBulkFurnitureFloorRemoveEvent(
        event: BulkFurnitureFloorRemoveEvent
    ): void {
        const parser = event.getParser();
        if (!parser) return;
        ...
        // Prefer one pass: remove all, then refresh tile map once.
        const ids = items.map((i) => i.itemId);
        const userId = items[0]?.userId ?? -1;

        this.removeFloorItemsByIds(ids, userId, /*refreshMap=*/ true);
        console.log('BulkItemsRemoved');
    }
    public removeFloorItemsByIds = (
        itemIds: number[],
        userId: number = -1,
        refreshMap: boolean = true
    ): void => {
        ...
        // Drop pending entries first so they don't spawn on the next tick
        const instanceData = this.getRoomInstanceData(this._activeRoomId);
        if (instanceData) {
            for (const id of itemIds)
                instanceData.removePendingFunitureFloor(id);
        }

        // Remove objects from the scene graph quickly
        const inst = this.getRoomInstance(this._activeRoomId) as RoomInstance;
        if (inst) {
            const cat = RoomObjectCategory.FLOOR;

            for (const id of itemIds) {
                // removeRoomObject() is the lowest-level fast path without the extra animations
                inst.removeRoomObject(id, cat);

                // keep other listeners in sync if they listen to engine events
                if (this.events)
                ...
                // legacy/per-item UI hook if you need it
                window.dispatchEvent(
                    new CustomEvent('furni_remove', {
                        detail: {
                            itemId: id,
                            isExpired: false,
                            userId,
        ...
        // Refresh stacking map ONCE (important for perf)
        if (refreshMap)
            this.refreshTileObjectMap(
                this._activeRoomId,
                'RoomEngine.removeFloorItemsByIds'
            );

        // Canonical batch broadcast
        window.dispatchEvent(
            new CustomEvent('furni_bulk_remove', {
                detail: {
    ...
    /** Convenience single-item fast remove using the same path as batch. */
    public removeFloorItemById = (
        itemId: number,
        userId: number = -1,
        refreshMap: boolean = true
    ): void => {
        this.removeFloorItemsByIds([itemId], userId, refreshMap);
    };
    ...
    public onDispose(): void {
        ...
        if (this._roomSessionManager) {
            this._roomSessionManager.events.removeEventListener(
                RoomSessionEvent.STARTED,
                this.onRoomSessionEvent
            );
            this._roomSessionManager.events.removeEventListener(
    ...
    public getRoomInstance(roomId: number): IRoomInstance {
        return (
            (this._roomManager &&
                this._roomManager.getRoomInstance(this.getRoomId(roomId))) ||
            null
        );
    }
    ...
    public removeRoomInstance(roomId: number): void {
        ...
        if (instance) {
            this._roomManager &&
                this._roomManager.removeRoomInstance(this.getRoomId(roomId));
        }
        ...
        this.events.dispatchEvent(
            new RoomEngineEvent(RoomEngineEvent.DISPOSED, roomId)
        );
    }
    ...
    public createRoomInstance(roomId: number, roomMap: RoomMapData): void {
        ...
        if (!this._ready) {
            ...
            this.logger.warn(
                'Room Engine not initilized yet, can not create room. Room data stored for later initialization.'
            );
        ...
        if (!roomMap) {
            this.logger.warn(
                'Room property messages received before floor height map, will initialize when floor height map received.'
            );
        ...
        const instance = this.setupRoomInstance(
            roomId,
            roomMap,
            floorType,
            wallType,
            landscapeType,
            this.getRoomInstanceModelName(roomId)
        );
        ...
        this.events.dispatchEvent(
            new RoomEngineEvent(RoomEngineEvent.INITIALIZED, roomId)
        );
    }
    ...
    private setupRoomInstance(
        ...
        const instance = this._roomManager.createRoomInstance(
        ...
        const roomObject = instance.createRoomObjectAndInitalize(
        ...
        if (roomMap) {
            instance.model.setValue(
                RoomVariableEnum.RESTRICTS_DRAGGING,
                roomMap.restrictsDragging
            );
            instance.model.setValue(
                RoomVariableEnum.RESTRICTS_SCALING,
                roomMap.restrictsScaling
            );
            instance.model.setValue(
                RoomVariableEnum.RESTRICTED_SCALE,
                roomMap.restrictedScale
            );
            ...
            if (dimensions) {
                ...
                if (roomObject && roomObject.model)
                    roomObject.model.setValue(
                        RoomObjectVariable.ROOM_RANDOM_SEED,
                        seed
                    );
            }
        }
        ...
        if (logic) {
            ...
            if (floorType) {
                logic.processUpdateMessage(
                    new ObjectRoomUpdateMessage(
                        ObjectRoomUpdateMessage.ROOM_FLOOR_UPDATE,
                        floorType
                    )
                );
                instance.model.setValue(
                    RoomObjectVariable.ROOM_FLOOR_TYPE,
                    floorType
                );
            }
            ...
            if (wallType) {
                logic.processUpdateMessage(
                    new ObjectRoomUpdateMessage(
                        ObjectRoomUpdateMessage.ROOM_WALL_UPDATE,
                        wallType
                    )
                );
                instance.model.setValue(
                    RoomObjectVariable.ROOM_WALL_TYPE,
                    wallType
                );
            }
            ...
            if (landscapeType) {
                logic.processUpdateMessage(
                    new ObjectRoomUpdateMessage(
                        ObjectRoomUpdateMessage.ROOM_LANDSCAPE_UPDATE,
                        landscapeType
                    )
                );
                instance.model.setValue(
                    RoomObjectVariable.ROOM_LANDSCAPE_TYPE,
                    landscapeType
                );
            }
        }
            ...
            while (doorIndex < roomMap.doors.length) {
                ...
                if (door) {
                    ...
                    if (doorDir === 90 || doorDir === 180) {
                        if (doorDir === 90) {
                            instance.model.setValue(
                                RoomObjectVariable.ROOM_DOOR_X,
                                doorX - 0.5
                            );
                            instance.model.setValue(
                                RoomObjectVariable.ROOM_DOOR_Y,
                                doorY
                            );
                        }
                        ...
                        instance.model.setValue(
                            RoomObjectVariable.ROOM_DOOR_Z,
                            doorZ
                        );
                        instance.model.setValue(
                            RoomObjectVariable.ROOM_DOOR_DIR,
                            doorDir
                        );
                    }
                }
        ...
        instance.createRoomObjectAndInitalize(
            RoomEngine.CURSOR_OBJECT_ID,
            RoomEngine.CURSOR_OBJECT_TYPE,
            RoomObjectCategory.CURSOR
        );
        if (Nitro.instance.getConfiguration('enable.avatar.arrow', false))
            instance.createRoomObjectAndInitalize(
    ...
    public getRoomInstanceDisplay(
        ...
        renderer.roomObjectVariableAccurateZ =
            RoomObjectVariable.OBJECT_ACCURATE_Z_VALUE;
        ...
        const restrictedScaling = instance.model.getValue<boolean>(
            RoomVariableEnum.RESTRICTS_SCALING
        );

        if (restrictedScaling) {
            let restrictedScale = instance.model.getValue<number>(
                RoomVariableEnum.RESTRICTED_SCALE
            );
        ...
        if (canvas.geometry) {
            canvas.geometry.z_scale = instance.model.getValue(
                RoomVariableEnum.ROOM_Z_SCALE
            );

            const doorX = instance.model.getValue<number>(
                RoomObjectVariable.ROOM_DOOR_X
            );
            const doorY = instance.model.getValue<number>(
                RoomObjectVariable.ROOM_DOOR_Y
            );
            const doorZ = instance.model.getValue<number>(
                RoomObjectVariable.ROOM_DOOR_Z
            );
            const doorDirection = instance.model.getValue<number>(
                RoomObjectVariable.ROOM_DOOR_DIR
            );
    ...
    public setRoomInstanceRenderingCanvasMask(
        roomId: number,
        canvasId: number,
        flag: boolean
    ): void {
        const roomCanvas = this.getRoomInstanceRenderingCanvas(
            roomId,
            canvasId
        );
    ...
    public setRoomInstanceRenderingCanvasScale(
        roomId: number,
        canvasId: number,
        scale: number,
        point: Point = null,
        ...
        if (roomCanvas) {
            ...
            this.events.dispatchEvent(
                new RoomEngineEvent(RoomEngineEvent.ROOM_ZOOMED, roomId)
            );
        }
    }

    public getRoomInstanceRenderingCanvas(
        roomId: number,
        canvasId: number = -1
    ...
    public getActiveRoomInstanceRenderingCanvas(): IRoomRenderingCanvas {
        return this.getRoomInstanceRenderingCanvas(
            this._activeRoomId,
            this._activeRoomActiveCanvas
        );
    }

    public getRoomInstanceRenderingCanvasOffset(
        roomId: number,
        canvasId: number = -1
    ): Point {
        if (canvasId === -1) canvasId = this._activeRoomActiveCanvas;

        const renderingCanvas = this.getRoomInstanceRenderingCanvas(
            roomId,
            canvasId
        );
        ...
        return new Point(
            renderingCanvas.screenOffsetX,
            renderingCanvas.screenOffsetY
        );
    }

    public setRoomInstanceRenderingCanvasOffset(
        roomId: number,
        canvasId: number,
        point: Point
    ): boolean {
    ...
    public getRoomInstanceRenderingCanvasScale(
        roomId: number = -1000,
        canvasId: number = -1
    ): number {
    ...
    public initializeRoomInstanceRenderingCanvas(
        roomId: number,
        canvasId: number,
        width: number,
        height: number
    ...
    public getRoomInstanceGeometry(
        roomId: number,
        canvasId: number = -1
    ): IRoomGeometry {
        const instance = this.getRoomInstance(roomId);
    ...
    public updateRoomInstancePlaneVisibility(
        roomId: number,
        wallVisible: boolean,
        ...
        object.processUpdateMessage(
            new ObjectRoomPlaneVisibilityUpdateMessage(
                ObjectRoomPlaneVisibilityUpdateMessage.WALL_VISIBILITY,
                wallVisible
            )
        );
        object.processUpdateMessage(
            new ObjectRoomPlaneVisibilityUpdateMessage(
                ObjectRoomPlaneVisibilityUpdateMessage.FLOOR_VISIBILITY,
                floorVisible
            )
        );
    ...
    public updateRoomInstancePlaneThickness(
        roomId: number,
        ...
        object.processUpdateMessage(
            new ObjectRoomPlanePropertyUpdateMessage(
                ObjectRoomPlanePropertyUpdateMessage.WALL_THICKNESS,
                wallThickness
            )
        );
        object.processUpdateMessage(
            new ObjectRoomPlanePropertyUpdateMessage(
                ObjectRoomPlanePropertyUpdateMessage.FLOOR_THICKNESS,
                floorThickness
            )
        );
    ...
    public updateRoomInstancePlaneType(
        roomId: number,
        floorType: string = null,
        wallType: string = null,
        landscapeType: string = null,
        _arg_5: boolean = false
    ): boolean {
    ...
    public updateObjectRoomColor(
        ...
        const event = new ObjectRoomColorUpdateMessage(
            ObjectRoomColorUpdateMessage.BACKGROUND_COLOR,
            color,
            light,
            backgroundOnly
        );
        ...
        this.events.dispatchEvent(
            new RoomBackgroundColorEvent(roomId, color, light, backgroundOnly)
        );
    ...
    public addRoomInstanceFloorHole(roomId: number, objectId: number): void {
        ...
        if (
            ...
            const sizeX = roomObject.model.getValue<number>(
                RoomObjectVariable.FURNITURE_SIZE_X
            );
            const sizeY = roomObject.model.getValue<number>(
                RoomObjectVariable.FURNITURE_SIZE_Y
            );

            roomOwnObject.processUpdateMessage(
                new ObjectRoomFloorHoleUpdateMessage(
    ...
    public removeRoomInstanceFloorHole(roomId: number, objectId: number): void {
        ...
        if (roomOwnObject) {
            roomOwnObject.processUpdateMessage(
                new ObjectRoomFloorHoleUpdateMessage(
                    ObjectRoomFloorHoleUpdateMessage.REMOVE,
                    objectId
                )
            );
        }
    ...
    public setRoomEngineGameMode(roomId: number, isPlaying: boolean): void {
        ...
        if (mode === 0) {
            this.events.dispatchEvent(
                new RoomEngineEvent(RoomEngineEvent.NORMAL_MODE, roomId)
            );
        } else {
            this.events.dispatchEvent(
                new RoomEngineEvent(RoomEngineEvent.GAME_MODE, roomId)
            );
        }
    }
    ...
    public isRoomIdPlayingGame(roomId: number): boolean {
        ...
        return (
            roomInstance.model.getValue<number>(
                RoomVariableEnum.IS_PLAYING_GAME
            ) > 0
        );
    }
    ...
    private processPendingFurniture(): void {
        ...
        for (const instanceData of this._roomInstanceDatas.values()) {
            ...
            while (
                (pendingData = instanceData.getNextPendingFurnitureFloor())
            ) {
                furnitureAdded = this.processPendingFurnitureFloor(
                    instanceData.roomId,
                    pendingData.id,
                    pendingData
                );
            ...
            while (
                ...
                furnitureAdded = this.processPendingFurnitureWall(
                    instanceData.roomId,
                    pendingData.id,
                    pendingData
                );
            ...
            if (furnitureAdded && this._roomManager) {
                const roomInstance = this._roomManager.getRoomInstance(
                    this.getRoomId(instanceData.roomId)
                ) as RoomInstance;
    ...
    public onRoomEngineInitalized(flag: boolean): void {
        ...
        this.events.dispatchEvent(
            new RoomEngineEvent(RoomEngineEvent.ENGINE_INITIALIZED, 0)
        );
    ...
    private processPendingFurnitureFloor(
        ...
        if (model) {
            model.setValue(
                RoomObjectVariable.FURNITURE_COLOR,
                this.getFurnitureFloorColorIndex(data.typeId)
            );
            model.setValue(RoomObjectVariable.FURNITURE_TYPE_ID, data.typeId);
            model.setValue(
                RoomObjectVariable.FURNITURE_AD_URL,
                this.getRoomObjectAdUrl(data.type)
            );
            model.setValue(
                RoomObjectVariable.FURNITURE_REAL_ROOM_OBJECT,
                data.realRoomObject ? 1 : 0
            );
            model.setValue(
                RoomObjectVariable.FURNITURE_EXPIRY_TIME,
                data.expiryTime
            );
            model.setValue(
                RoomObjectVariable.FURNITURE_EXPIRTY_TIMESTAMP,
                Nitro.instance.time
            );
            model.setValue(
                RoomObjectVariable.FURNITURE_USAGE_POLICY,
                data.usagePolicy
            );
        ...
        if (
            ...
            selectedRoomObjectData.category === RoomObjectCategory.FLOOR
        ) {
            this.selectRoomObject(roomId, id, RoomObjectCategory.FLOOR);
        }

        if (object.isReady && data.synchronized)
            this.addObjectToTileMap(roomId, object);
    ...
    private processPendingFurnitureWall(
        ...
        if (model) {
            model.setValue(
                RoomObjectVariable.FURNITURE_COLOR,
                this.getFurnitureWallColorIndex(data.typeId)
            );
            model.setValue(RoomObjectVariable.FURNITURE_TYPE_ID, data.typeId);
            model.setValue(
                RoomObjectVariable.FURNITURE_AD_URL,
        ...
        if (
            ...
            selectedRoomObjectData.category === RoomObjectCategory.WALL
        ) {
    ...
    private updateRoomCameras(time: number): void {
        for (const instanceData of this._roomInstanceDatas.values()) {
            ...
            const object = this.getRoomObject(
                instanceData.roomId,
                camera.targetId,
                camera.targetCategory
            );
            ...
            if (
                instanceData.roomId !== this._activeRoomId ||
                !this._activeRoomIsDragged
            ) {
        ...
        if (this._activeRoomWasDragged) {
            ...
            if (renderingCanvas)
                this.setRoomInstanceRenderingCanvasOffset(
                    this._activeRoomId,
                    1,
                    new Point(
                        renderingCanvas.screenOffsetX + this._activeRoomDragX,
                        renderingCanvas.screenOffsetY + this._activeRoomDragY
                    )
                );
    ...
    private updateRoomCamera(
        ...
        if (
            ...
            !Vector3d.isEqual(objectLocation, roomCamera.targetObjectLoc) ||
            roomCamera.isMoving
        ) {
            ...
            const _local_22 = 2;
            let _local_23 = new Point(
                _local_15.x - _local_20,
                _local_15.y - _local_21
            );
            const _local_24 = roomGeometry.scale / Math.sqrt(2);
            const _local_25 = _local_24 / 2;
            const _local_26 = new Matrix();
            _local_26.rotate(
                (-(roomGeometry.direction.x + 90) / 180) * Math.PI
            );
            ...
            let _local_31 = 0;
            let _local_32 = 0;
            let _local_33 = roomGeometry.getScreenPoint(
                new Vector3d(_local_20, _local_21, _local_22)
            );
            ...
            if (bounds) {
                ...
                if (bounds.width > 1 && bounds.height > 1) {
                    ...
                    _local_31 =
                        ...
                        _local_24;
                    _local_30 =
                        (bounds.top - _local_33.y - roomGeometry.scale * 0.5) /
                        _local_25;
                    _local_32 =
                        (bounds.bottom -
                            _local_33.y +
                            roomGeometry.scale * 0.5) /
                        _local_25;
                } else {
            ...
            canvasRectangle.height =
                canvasRectangle.height * (1 - (_local_40 + _local_41));
            ...
            if (_local_33) {
                ...
                if (
                    ((_local_33.x < canvasRectangle.left ||
                        _local_33.x > canvasRectangle.right) &&
                        !roomCamera.centeredLocX) ||
                    ((_local_33.y < canvasRectangle.top ||
                        _local_33.y > canvasRectangle.bottom) &&
                        !roomCamera.centeredLocY) ||
                    (_local_36 &&
                        !roomCamera.centeredLocX &&
                        !(roomCamera.screenWd == width)) ||
                    (_local_37 &&
            ...
            if (this.useOffsetScrolling) {
                this.setRoomInstanceRenderingCanvasOffset(
                    this.activeRoomId,
                    1,
                    new Point(-roomCamera.location.x, -roomCamera.location.y)
                );
            } else {
    ...
    public getRoomObjectBoundingRectangle(
        roomId: number,
        objectId: number,
        category: number,
        canvasId: number
    ...
    public getFurnitureWallName(typeId: number, extra: string = null): string {
        ...
        return this._roomContentLoader.getFurnitureWallNameForTypeId(
            typeId,
            extra
        );
    }
    ...
    public setSelectedRoomObjectData(
        roomId: number,
    ...
    public setPlacedRoomObjectData(
        roomId: number,
        data: SelectedRoomObjectData
    ): void {
        const instanceData = this.getRoomInstanceData(roomId);
    ...
    public cancelRoomObjectPlacement(): void {
        ...
        this._roomObjectEventHandler.cancelRoomObjectPlacement(
            this._activeRoomId
        );
    }

    public getFurnitureStackingHeightMap(
        roomId: number
    ...
    public setFurnitureStackingHeightMap(
        roomId: number,
        heightMap: FurnitureStackingHeightMap
    ): void {
        const instanceData = this.getRoomInstanceData(roomId);
    ...
    private createRoomObjectAndInitialize(
        ...
        return instance.createRoomObjectAndInitalize(
            objectId,
            type,
            category
        ) as IRoomObjectController;
    ...
    public getObject(
        ...
        if (this._roomManager)
            roomInstance = this._roomManager.getRoomInstance(roomId);

        if (!roomInstance) return null;

        let roomObject = roomInstance.getRoomObject(
            objectId,
            category
        ) as IRoomObjectController;
        ...
        if (!roomObject) {
            switch (category) {
                case RoomObjectCategory.FLOOR:
                    this.processPendingFurnitureFloor(
                        this.getRoomIdFromString(roomId),
                        objectId,
                        null
                    );

                    roomObject = roomInstance.getRoomObject(
                        objectId,
                        category
                    ) as IRoomObjectController;
                    break;
                case RoomObjectCategory.WALL:
                    this.processPendingFurnitureWall(
                        this.getRoomIdFromString(roomId),
                        objectId,
                        null
                    );
    ...
    public getRoomObjectByIndex(
        ...
        return instance.getRoomObjectByIndex(
            index,
            category
        ) as IRoomObjectController;
    }
    ...
    public createRoomObjectUser(
        roomId: number,
        objectId: number,
        type: string
    ): IRoomObjectController {
        return this.createRoomObjectAndInitialize(
            roomId,
            objectId,
            type,
            RoomObjectCategory.UNIT
    ...
    public removeRoomObjectFloor(
        ...
        if (
            ...
            userId === this._sessionDataManager.userId &&
            !FurniId.isBuilderClubId(objectId)
        ) {
            ...
            if (roomObject) {
                ...
                if (screenLocation) {
                    ...
                    if (!disabledPickingAnimation) {
                        const typeId = roomObject.model.getValue<number>(
                            RoomObjectVariable.FURNITURE_TYPE_ID
                        );
                        const extras = roomObject.model.getValue<string>(
                            RoomObjectVariable.FURNITURE_EXTRAS
                        );
                        const dataKey = roomObject.model.getValue<number>(
                            RoomObjectVariable.FURNITURE_DATA_FORMAT
                        );
                        const objectData = ObjectDataFactory.getData(dataKey);
                        const icon = this.getFurnitureFloorIcon(
                            typeId,
                            null,
                            extras,
                            objectData
        ...
        if (_arg_4)
            this.refreshTileObjectMap(
                roomId,
                'RoomEngine.disposeObjectFurniture()'
            );
    }

    public getRoomObjectWall(
        roomId: number,
        objectId: number
    ): IRoomObjectController {
        return this.getObject(
        ...
        if (
            ...
            if (
                ...
                if (screenLocation) {
                    ...
                    const objectData = roomObject.model.getValue<string>(
                        RoomObjectVariable.FURNITURE_DATA
                    );
                    const icon = this.getFurnitureWallIcon(
                        typeId,
                        null,
                        objectData
                    ).data;
    ...
    public addFurnitureFloor(
        ...
        const furnitureData = new RoomFurnitureData(
            id,
            typeId,
            null,
            location,
    ...
    public updateRoomObjectFloor(
        ...
        object.processUpdateMessage(
            new RoomObjectUpdateMessage(location, direction)
        );
        object.processUpdateMessage(
            new ObjectDataUpdateMessage(state, data, extra)
        );
    ...
    public updateRoomObjectWallItemData(
        ...
        object.logic.processUpdateMessage(
            new ObjectItemDataUpdateMessage(data)
        );

        return true;
    }

    public updateRoomObjectFloorHeight(
        roomId: number,
        objectId: number,
        height: number
    ): boolean {
        ...
        object.processUpdateMessage(
            new ObjectHeightUpdateMessage(null, null, height)
        );

        return true;
    }

    public updateRoomObjectFloorExpiration(
        roomId: number,
        objectId: number,
        expires: number
    ): boolean {
    ...
    public updateRoomObjectWallExpiration(
        roomId: number,
        objectId: number,
        expires: number
    ): boolean {
    ...
    public updateRoomObjectMask(
        ...
        if (roomObject && roomObject.model) {
            if (
                roomObject.model.getValue<number>(
                    RoomObjectVariable.FURNITURE_USES_PLANE_MASK
                ) > 0
            ) {
                const maskType = roomObject.model.getValue<string>(
                    RoomObjectVariable.FURNITURE_PLANE_MASK_TYPE
                );
                const location = roomObject.getLocation();
                ...
                if (_arg_3)
                    ...
                    maskUpdate = new ObjectRoomMaskUpdateMessage(
                        ObjectRoomMaskUpdateMessage.REMOVE_MASK,
                        maskName
                    );
            }
        } else {
    ...
    public updateRoomObjectWallLocation(
        ...
        if (roomObject.logic)
            roomObject.logic.processUpdateMessage(
                new ObjectMoveUpdateMessage(location, null, null)
            );
    ...
    public addRoomObjectUser(
        ...
        if (objectType === RoomObjectUserType.PET)
        ...
        object.processUpdateMessage(
            new ObjectAvatarUpdateMessage(
                this.fixedUserLocation(roomId, location),
                null,
                direction,
                headDirection,
                false,
        ...
        if (figure)
            object.processUpdateMessage(
                new ObjectAvatarFigureUpdateMessage(figure)
            );
    ...
    public updateRoomObjectUserLocation(
        roomId: number,
        objectId: number,
        location: IVector3D,
        targetLocation: IVector3D,
        canStandUp: boolean = false,
        baseY: number = 0,
        direction: IVector3D = null,
        headDirection: number = NaN
    ): boolean {
        ...
        if (isNaN(headDirection))
            headDirection = object.model.getValue<number>(
                RoomObjectVariable.HEAD_DIRECTION
            );
        ...
        const roomSession =
            (this._roomSessionManager &&
                this._roomSessionManager.getSession(roomId)) ||
            null;

        if (roomSession && roomSession.ownRoomIndex === objectId) {
            this._logicFactory.events.dispatchEvent(
                new RoomToObjectOwnAvatarMoveEvent(
                    RoomToObjectOwnAvatarMoveEvent.ROAME_MOVE_TO,
                    targetLocation
                )
            );
        }
    ...
    private fixedUserLocation(roomId: number, location: IVector3D): IVector3D {
        ...
        if (
            Math.abs(_local_5 - _local_6) < 0.1 &&
            Math.abs(_local_6 - _local_7) < 0.1
        ) {
    ...
    public updateRoomObjectUserAction(
        roomId: number,
        objectId: number,
        action: string,
        value: number,
        ...
        switch (action) {
            ...
            case RoomObjectVariable.FIGURE_CARRY_OBJECT:
                message = new ObjectAvatarCarryObjectUpdateMessage(
                    value,
                    parameter
                );
                break;
    ...
    public updateRoomObjectUserFigure(
        ...
        object.processUpdateMessage(
            new ObjectAvatarFigureUpdateMessage(
                figure,
                gender,
                subType,
                isRiding
    ...
    public updateRoomObjectUserFlatControl(
        roomId: number,
        objectId: number,
        level: string
    ): boolean {
        ...
        object.processUpdateMessage(
            new ObjectAvatarFlatControlUpdateMessage(parseInt(level))
        );

        return true;
    }

    public updateRoomObjectUserEffect(
        roomId: number,
        objectId: number,
        effectId: number,
        delay: number = 0
        ...
        object.processUpdateMessage(
            new ObjectAvatarEffectUpdateMessage(effectId, delay)
        );

        return true;
    }

    public updateRoomObjectUserGesture(
        roomId: number,
        objectId: number,
        gestureId: number
    ): boolean {
        ...
        object.processUpdateMessage(
            new ObjectAvatarGestureUpdateMessage(gestureId)
        );

        return true;
    }

    public updateRoomObjectUserPetGesture(
        roomId: number,
        objectId: number,
        gesture: string
    ): boolean {
        ...
        object.processUpdateMessage(
            new ObjectAvatarPetGestureUpdateMessage(gesture)
        );

        return true;
    }

    public updateRoomObjectUserPosture(
        roomId: number,
        objectId: number,
        type: string,
        parameter: string = null
        ...
        object.processUpdateMessage(
            new ObjectAvatarPostureUpdateMessage(type, parameter)
        );
    ...
    public objectInitialized(
        ...
        if (object && object.model && object.logic) {
            const dataFormat = object.model.getValue<number>(
                RoomObjectVariable.FURNITURE_DATA_FORMAT
            );
            ...
            if (!isNaN(dataFormat)) {
                ...
                object.processUpdateMessage(
                    new ObjectDataUpdateMessage(object.getState(0), data)
                );
            }
    ...
    public changeObjectModelData(
        ...
        const roomObject = this.getObject(
            this.getRoomId(roomId),
            objectId,
            category
        );
        ...
        const message = new ObjectModelDataUpdateMessage(
            numberKey,
            numberValue
        );
    ...
    public changeObjectState(
        ...
        let stateIndex = roomObject.model.getValue<number>(
            RoomObjectVariable.FURNITURE_AUTOMATIC_STATE_INDEX
        );
        ...
        const objectDataKey = roomObject.model.getValue<number>(
            RoomObjectVariable.FURNITURE_DATA_FORMAT
        );
        ...
        if (roomObject.logic)
            roomObject.logic.processUpdateMessage(
                new ObjectDataUpdateMessage(stateIndex, objectData)
            );
    }

    public loadRoomObjectBadgeImage(
        roomId: number,
        objectId: number,
        objectCategory: number,
        badgeId: string,
        groupBadge: boolean = true
    ): void {
        ...
        if (roomId === 0) {
            const room = this._roomManager.getRoomInstance(
                RoomEngine.TEMPORARY_ROOM
            );

            if (room)
                roomObject = room.getRoomObject(
                    objectId,
                    objectCategory
                ) as IRoomObjectController;
        } else {
        ...
        let badgeName = groupBadge
            ? this._sessionDataManager.loadGroupBadgeImage(badgeId)
            : this._sessionDataManager.loadBadgeImage(badgeId);
        ...
        if (!badgeName) {
            ...
            if (!this._badgeListenerObjects.size) {
                this._sessionDataManager.events.addEventListener(
                    BadgeImageReadyEvent.IMAGE_READY,
                    this.onBadgeImageReadyEvent
                );
            }
            ...
            listeners.push(
                new RoomObjectBadgeImageAssetListener(roomObject, groupBadge)
            );
        ...
        roomObject.logic.processUpdateMessage(
            new ObjectGroupBadgeUpdateMessage(badgeId, badgeName)
        );
    }
    ...
    private onBadgeImageReadyEvent(k: BadgeImageReadyEvent): void {
        ...
        const listeners =
            this._badgeListenerObjects &&
            this._badgeListenerObjects.get(k.badgeId);
        ...
        for (const listener of listeners) {
            ...
            this.putBadgeInObjectAssets(
                listener.object,
                k.badgeId,
                listener.groupBadge
            );

            const badgeName = listener.groupBadge
                ? this._sessionDataManager.loadGroupBadgeImage(k.badgeId)
                : this._sessionDataManager.loadBadgeImage(k.badgeId);

            if (listener.object && listener.object.logic)
                listener.object.logic.processUpdateMessage(
                    new ObjectGroupBadgeUpdateMessage(k.badgeId, badgeName)
                );
        }
        ...
        if (!this._badgeListenerObjects.size) {
            this._sessionDataManager.events.removeEventListener(
                BadgeImageReadyEvent.IMAGE_READY,
                this.onBadgeImageReadyEvent
            );
        }
    ...
    private putBadgeInObjectAssets(
        ...
        badgeId: string,
        groupBadge: boolean = false
    ): void {
        if (!this._roomContentLoader || !this._sessionDataManager) return;

        const badgeName = groupBadge
            ? this._sessionDataManager.loadGroupBadgeImage(badgeId)
            : this._sessionDataManager.loadBadgeImage(badgeId);
        const badgeImage = groupBadge
            ? this._sessionDataManager.getGroupBadgeImage(badgeId)
            : this._sessionDataManager.getBadgeImage(badgeId);

        if (badgeImage)
            this._roomContentLoader.addAssetToCollection(
                object.type,
                badgeName,
                badgeImage,
                false
    ...
    public dispatchMouseEvent(
        ...
        const canvas = this.getRoomInstanceRenderingCanvas(
            this._activeRoomId,
            canvasId
        );
        ...
        const sprite = this.getOverlayIconSprite(
            overlay,
            RoomEngine.OBJECT_ICON_SPRITE
        );
        ...
        if (
            ...
            if (
                ...
                if (type === MouseEventType.MOUSE_CLICK) {
                    ...
                    else if (type === MouseEventType.MOUSE_DOWN)
                        eventType = RoomObjectMouseEvent.MOUSE_DOWN;
                    else if (type === MouseEventType.MOUSE_DOWN_LONG)
                        eventType = RoomObjectMouseEvent.MOUSE_DOWN_LONG;
                    else if (type === MouseEventType.MOUSE_UP)
                        eventType = RoomObjectMouseEvent.MOUSE_UP;
                }
        ...
        if (type === MouseEventType.MOUSE_DOWN) {
            if (!altKey && !ctrlKey && !shiftKey && !this.isDecorating) {
                if (this._roomAllowsDragging) {
                    ...
                    this._activeRoomWasDragged = false;
                    this._activeRoomDragStartX =
                        this._activeRoomActiveCanvasMouseX;
                    this._activeRoomDragStartY =
                        this._activeRoomActiveCanvasMouseY;
                }
            }
        } else if (type === MouseEventType.MOUSE_UP) {
            if (this._activeRoomIsDragged) {
                ...
                if (this._activeRoomWasDragged) {
                    const instanceData = this.getRoomInstanceData(
                        this._activeRoomId
                    );
                        ...
                        if (camera) {
                            if (this.useOffsetScrolling) {
                                ...
                                camera.resetLocation(
                                    new Vector3d(
                                        -canvas.screenOffsetX,
                                        -canvas.screenOffsetY
                                    )
                                );
                            }
            ...
            if (this._activeRoomIsDragged) {
                if (!this._activeRoomWasDragged) {
                    ...
                    if (
                        offsetX <= -RoomEngine.DRAG_THRESHOLD ||
                        offsetX >= RoomEngine.DRAG_THRESHOLD ||
                        offsetY <= -RoomEngine.DRAG_THRESHOLD ||
                        offsetY >= RoomEngine.DRAG_THRESHOLD
                    ) {
    ...
    private setMouseButton(
        ...
        if (
            ...
            session.controllerLevel >= RoomControllerLevel.GUEST
        ) {
            const instanceData = this.getRoomInstanceData(roomId);

            if (instanceData) {
                if (
                    instanceData.addButtonMouseCursorOwner(
                        category + '_' + objectId
                    )
                )
    ...
    private setMouseDefault(
        ...
        if (instanceData) {
            if (
                instanceData.removeButtonMouseCursorOwner(
                    category + '_' + objectId
                )
            )
                this._mouseCursorUpdate = true;
    ...
    public processRoomObjectOperation(
        objectId: number,
        category: number,
        operation: string
    ): boolean {
        ...
        this._roomObjectEventHandler.modifyRoomObject(
            this._activeRoomId,
            objectId,
            category,
            operation
    ...
    public modifyRoomObjectDataWithMap(
        objectId: number,
        category: number,
        operation: string,
        ...
        this._roomObjectEventHandler.modifyRoomObjectDataWithMap(
            this._activeRoomId,
            objectId,
            category,
            operation,
    ...
    public modifyRoomObjectData(
        ...
        this._roomObjectEventHandler.modifyWallItemData(
            this._activeRoomId,
            objectId,
            colorHex,
            data
    ...
    public processRoomObjectPlacement(
        ...
        if (
            !roomInstance ||
            roomInstance.model.getValue<number>(
                RoomVariableEnum.ROOM_IS_PUBLIC
            ) !== 0
        )
            return false;
        ...
        return this._roomObjectEventHandler.processRoomObjectPlacement(
            placementSource,
            this._activeRoomId,
            id,
            category,
    ...
    public getRoomObjectScreenLocation(
        roomId: number,
        objectId: number,
        objectType: number,
        canvasId: number = -1
        ...
        screenPoint.x +=
            renderingCanvas.width / 2 + renderingCanvas.screenOffsetX;
        screenPoint.y +=
            renderingCanvas.height / 2 + renderingCanvas.screenOffsetY;
    ...
    public selectRoomObject(
        ...
        this._roomObjectEventHandler.setSelectedObject(
            roomId,
            objectId,
            objectCategory
        );
    ...
    public setObjectMoverIconSprite(
        objectId: number,
        category: number,
        _arg_3: boolean,
        ...
        if (_arg_3) {
            imageResult = this.getRoomObjectImage(
                this._activeRoomId,
                objectId,
                category,
                new Vector3d(),
            ...
            if (this._roomContentLoader) {
                if (category === RoomObjectCategory.FLOOR) {
                    type =
                        this._roomContentLoader.getFurnitureFloorNameForTypeId(
                            objectId
                        );
                    colorIndex =
                        this._roomContentLoader.getFurnitureFloorColorIndex(
                            objectId
                        );
                } else if (category === RoomObjectCategory.WALL) {
                    type =
                        this._roomContentLoader.getFurnitureWallNameForTypeId(
                            objectId,
                            instanceData
                        );
                    colorIndex =
                        this._roomContentLoader.getFurnitureWallColorIndex(
                            objectId
                        );
                }
                ...
                if (category === RoomObjectCategory.UNIT) {
                    ...
                    if (type === 'pet') {
                        ...
                        imageResult = this.getRoomObjectPetImage(
                        ...
                        imageResult = this.getGenericRoomObjectImage(
                            type,
                            instanceData,
                            new Vector3d(180),
                            64,
        ...
        const _local_15 = this.addOverlayIconSprite(
            overlay,
            RoomEngine.OBJECT_ICON_SPRITE,
            imageResult.data,
            scale
        ...
        if (_local_15) {
            _local_15.x =
                this._activeRoomActiveCanvasMouseX - imageResult.data.width / 2;
            _local_15.y =
                this._activeRoomActiveCanvasMouseY -
                imageResult.data.height / 2;
        }
    }

    public getRoomObjectImage(
        roomId: number,
        objectId: number,
        category: number,
        direction: IVector3D,
        ...
        if (roomInstance) {
            ...
            if (roomObject && roomObject.model) {
                ...
                switch (category) {
                    case RoomObjectCategory.FLOOR:
                    case RoomObjectCategory.WALL: {
                        color = roomObject.model
                            .getValue<number>(
                                RoomObjectVariable.FURNITURE_COLOR
                            )
                            .toString();
                        extras = roomObject.model.getValue<string>(
                            RoomObjectVariable.FURNITURE_EXTRAS
                        );

                        const dataFormat = roomObject.model.getValue<number>(
                            RoomObjectVariable.FURNITURE_DATA_FORMAT
                        );
                        ...
                        if (dataFormat !== LegacyDataType.FORMAT_KEY) {
                            ...
                            data.initializeFromRoomObjectModel(
                                roomObject.model
                            );
                        }
                    ...
                    case RoomObjectCategory.UNIT:
                        color = roomObject.model.getValue<string>(
                            RoomObjectVariable.FIGURE
                        );
                        break;
                }
            }
        ...
        return this.getGenericRoomObjectImage(
            type,
            color,
            direction,
            scale,
    ...
    public getFurnitureFloorIconUrl(typeId: number): string {
        ...
        if (this._roomContentLoader) {
            type =
                this._roomContentLoader.getFurnitureFloorNameForTypeId(typeId);
            color = this._roomContentLoader
    ...
    public getFurnitureFloorIcon(
        typeId: number,
        listener: IGetImageListener,
        extras: string = null,
        objectData: IObjectData = null
    ): ImageResult {
        return this.getFurnitureFloorImage(
            typeId,
            new Vector3d(),
            1,
            listener,
    ...
    public getFurnitureWallIconUrl(
        ...
        if (this._roomContentLoader) {
            type = this._roomContentLoader.getFurnitureWallNameForTypeId(
                typeId,
                extra
            );
            color = this._roomContentLoader
    ...
    public getFurnitureWallIcon(
        typeId: number,
        listener: IGetImageListener,
        extras: string = null
    ): ImageResult {
        return this.getFurnitureWallImage(
            typeId,
            new Vector3d(),
            1,
            listener,
    ...
    public getFurnitureFloorImage(
        ...
        if (scale === 1 && listener) {
            return this.getGenericRoomObjectThumbnail(
                type,
                color,
                listener,
                extras,
    ...
    public getFurnitureWallImage(
        ...
        if (this._roomContentLoader) {
            type =
                this._roomContentLoader.getFurnitureWallNameForTypeId(typeId);
            color = this._roomContentLoader
                .getFurnitureWallColorIndex(typeId)
                .toString();
        }
    ...
    public getRoomObjectPetImage(
        ...
        bgColor: number = 0,
        customParts: PetCustomPart[] = null,
        posture: string = null
    ): ImageResult {
        let type: string = null;
    ...
    public getGenericRoomObjectImage(
        ...
        let roomInstance = this._roomManager.getRoomInstance(
            RoomEngine.TEMPORARY_ROOM
        );

        if (!roomInstance) {
            roomInstance = this._roomManager.createRoomInstance(
                RoomEngine.TEMPORARY_ROOM
            );
        ...
        const roomObject = roomInstance.createRoomObjectAndInitalize(
            objectId,
            type,
            objectCategory
        ) as IRoomObjectController;
        ...
        switch (objectCategory) {
            ...
            case RoomObjectCategory.UNIT:
                if (
                    ...
                    model.setValue(
                        RoomObjectVariable.PET_PALETTE_INDEX,
                        figureData.paletteId
                    );
                    model.setValue(
                        RoomObjectVariable.PET_COLOR,
                        figureData.color
                    );

                    if (figureData.headOnly)
                        model.setValue(RoomObjectVariable.PET_HEAD_ONLY, 1);

                    if (figureData.hasCustomParts) {
                        model.setValue(
                            RoomObjectVariable.PET_CUSTOM_LAYER_IDS,
                            figureData.customLayerIds
                        );
                        model.setValue(
                            RoomObjectVariable.PET_CUSTOM_PARTS_IDS,
                            figureData.customPartIds
                        );
                        model.setValue(
                            RoomObjectVariable.PET_CUSTOM_PALETTE_IDS,
                            figureData.customPaletteIds
                        );
                    }

                    if (posture)
                        model.setValue(
                            RoomObjectVariable.FIGURE_POSTURE,
                            posture
                        );
                }
                break;
        ...
        if (state > -1 || objectData) {
            if (objectData && objectData.getLegacyString() !== '') {
                roomObject.logic.processUpdateMessage(
                    new ObjectDataUpdateMessage(
                        parseInt(objectData.getLegacyString()),
                        objectData
                    )
                );
            } else {
                roomObject.logic.processUpdateMessage(
                    new ObjectDataUpdateMessage(state, objectData)
                );
            }
        }
    ...
    public getGenericRoomObjectThumbnail(
        type: string,
        param: string,
        listener: IGetImageListener,
        ...
        if (!asset && listener) {
            ...
            if (!contentListeners) {
                ...
                this._roomContentLoader.downloadImage(
                    objectId,
                    type,
                    param,
                    null
    ...
    public softRefreshRoom(roomId: number): void
{
...
const canvas = this.getRoomInstanceRenderingCanvas(roomId, canvasId);
const width = (canvas as any)?.width ?? 0;
const height = (canvas as any)?.height ?? 0;

const scale = this.getRoomInstanceRenderingCanvasScale(roomId, canvasId);
const offset = this.getRoomInstanceRenderingCanvasOffset(roomId, canvasId);

// Pull current map from existing instance (adjust property names if needed)
const roomInstance: any = (this as any).getRoomInstance?.(roomId) ||
(this as any)._roomManager?.getRoom?.(roomId);

const roomMap = roomInstance?.roomMap || roomInstance?.roomMapData || roomInstance?.data?.roomMap;
...
this.removeRoomInstance(roomId);
this.createRoomInstance(roomId, roomMap);

this.initializeRoomInstanceRenderingCanvas(roomId, canvasId, width, height);
this.setRoomInstanceRenderingCanvasScale(roomId, canvasId, scale, undefined, offset, true, false);
...
this.runUpdate();
    ...
    public initalizeTemporaryObjectsByType(
        type: string,
        _arg_2: boolean
    ): void {
        const roomInstance = this._roomManager.getRoomInstance(
        ...
        if (objectManager && objectManager.objects.length) {
            for (const roomObject of objectManager.objects.getValues()) {
                if (
                    ...
                    if (visualization) {
                        const imageScale = roomObject.model.getValue<number>(
                            RoomObjectVariable.IMAGE_QUERY_SCALE
                        );
                    ...
                    const imageListeners = this._imageCallbacks.get(
                        objectId.toString()
                    );
    ...
    private getRenderingCanvasOverlay(k: IRoomRenderingCanvas): NitroSprite {
        ...
        return (
            (displayObject.getChildByName(RoomEngine.OVERLAY) as NitroSprite) ||
            null
        );
    }
    ...
    public refreshTileObjectMap(k: number, _arg_2: string): void {
        ...
        if (tileObjectMap)
            tileObjectMap.populate(
                this.getRoomObjects(k, RoomObjectCategory.FLOOR)
            );
    }
    ...
    public getRenderRoomMessage(
        k: Rectangle,
        _arg_2: number,
        _arg_3: boolean = false,
        _arg_4: boolean = true,
        _arg_5: boolean = false,
        canvasId: number = -1
    ): IMessageComposer<unknown[]> {
        let canvas: IRoomRenderingCanvas = null;

        if (canvasId > -1) {
            canvas = this.getRoomInstanceRenderingCanvas(
                this._activeRoomId,
                canvasId
            );
        } else {
        ...
        if (
            !_arg_4 &&
            !(this._roomSessionManager.getSession(this._activeRoomId) == null)
        ) {
            _local_8 = this._roomSessionManager.getSession(
                this._activeRoomId
            ).ownRoomIndex;
        }
    ...
    public saveTextureAsScreenshot(
        texture: RenderTexture,
        saveAsThumbnail: boolean = false
    ): void {
        let composer: RenderRoomMessageComposer = null;

        if (saveAsThumbnail)
            composer = new RenderRoomThumbnailMessageComposer();
    ...
    public objectsInitialized(k: string): void {
        ...
        this.events.dispatchEvent(
            new RoomEngineEvent(RoomEngineEvent.OBJECTS_INITIALIZED, roomId)
        );
    }
    ...
    public getPetColorResult(
        petIndex: number,
        paletteIndex: number
    ): PetColorResult {
        if (!this._roomContentLoader) return null;

        return this._roomContentLoader.getPetColorResult(
            petIndex,
            paletteIndex
        );
    }

    public getPetColorResultsForTag(
        petIndex: number,
        tagName: string
    ): PetColorResult[] {
        if (!this._roomContentLoader) return null;

        return this._roomContentLoader.getPetColorResultsForTag(
            petIndex,
            tagName
        );
    }
    ...
    public deleteRoomObject(objectId: number, objectCategory: number): boolean {
        if (
            !this._roomObjectEventHandler ||
            objectCategory !== RoomObjectCategory.WALL
        )
            return false;

        return this._roomObjectEventHandler.deleteWallItem(
            this._activeRoomId,
            objectId
        );
    }
    ...
    public getRoomObjectCount(roomId: number, categoryId: number): number {
        ...
        return this._roomManager
            .getRoomInstance(roomId.toString())
            .getRoomObjectsForCategory(categoryId).length;
    }
}
